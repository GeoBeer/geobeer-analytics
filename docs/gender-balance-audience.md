...
For some time, we've been discussing the topic of diversity in the (Swiss) GIS world. We hence decided to have a closer look at the audience of GeoBeer events, as it would give us a first data point (admittedly, with a small sample) in this particular industry sector. We started out with the gender balance, since this was the one characteristic we could analyse rather straight forward with the GeoBeer event registration data. There are of course many more aspects to diversity than gender. However, so far we have not systematically collected data on other aspects, e.g. languages spoken, nationality, age, etc. 
We further felt the need to have some kind of a baseline, which is why we also had a look at other GIS channels in Switzerland like the [GEOWebforum](https://www.geowebforum.ch) or the [SwissGIS](https://twitter.com/rastrau/lists/swissgis) list on Twitter. 
